...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"UIImage+Category error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        NSLog(@"UIImage+Category error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"UIImage+Category error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (hasNewFunc) {
        vImage_Error error;
        error = vImageBuffer_InitWithCGImage(&effect, &format, NULL, imageRef, kvImagePrintDiagnosticsToConsole);
        if (error != kvImageNoError) {
            NSLog(@"UIImage+Category error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", error, self);
            return nil;
        }
        error = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (error != kvImageNoError) {
            NSLog(@"UIImage+Category error: vImageBuffer_Init returned error code %zi for inputImage: %@", error, self);
            return nil;
        }
    } else {
